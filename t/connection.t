...
throws_ok {
    MongoDB::Async::Connection->new(host => 'localhost', port => 1);
...
SKIP: {
    ...
    my $to = MongoDB::Async::Connection->new('timeout' => 1);
    $to = MongoDB::Async::Connection->new('timeout' => 123);
    $to = MongoDB::Async::Connection->new('timeout' => 2000000);

    # test conn format
    lives_ok {
        $conn = MongoDB::Async::Connection->new("host" => "mongodb://localhost:27017");
    } 'connected';
    ...
    lives_ok {
        $conn = MongoDB::Async::Connection->new("host" => "mongodb://localhost:27018,localhost:27019,localhost");
    } 'last in line';
}
...
# query_timeout
{
    ...
    my $conn2 = MongoDB::Async::Connection->new(auto_connect => 0);
    is($conn2->query_timeout, $timeout, 'query timeout');

    $MongoDB::Async::Cursor::timeout = 40;
    $conn2 = MongoDB::Async::Connection->new(auto_connect => 0);
