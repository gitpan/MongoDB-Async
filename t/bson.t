...
# id realloc
{
    ...
    my $med_str = "z" x 4014;
    $c->insert({'text' => $med_str, 'id2' => MongoDB::Async::OID->new});
    my $result = $c->find_one;
...

{
    ...
    my $id = $c->insert({"n" => undef,
                ...
                "a" => {"foo" => "bar",
                        "n" => undef,
                        "x" => MongoDB::Async::OID->new("49b6d9fb17330414a0c63102")},
                "d2" => DateTime->from_epoch(epoch => 1271079861),
                "regex" => qr/xtz/,
                "_id" => MongoDB::Async::OID->new("49b6d9fb17330414a0c63101"),
                "string" => "string"});
	...
	#   Failed test at t/bson.t line 73.
	#          got: '23.2345145200000012'
	#     expected: '23.23451452'
    ...
    is($obj->{'a'}->{'foo'}, 'bar');
    is($obj->{'a'}->{'n'}, undef);
    isa_ok($obj->{'a'}->{'x'}, 'MongoDB::Async::OID');
    isa_ok($obj->{'d2'}, 'DateTime');
    is($obj->{'d2'}->epoch, 1271079861);
    ok($obj->{'regex'});
    isa_ok($obj->{'_id'}, 'MongoDB::Async::OID');
    is($obj->{'_id'}, $id);
...
# MongoDB::Async::BSON::String type
{
    $MongoDB::Async::BSON::looks_like_number = 1;
    ...
    $c->insert({num => bless(\$num, "MongoDB::Async::BSON::String")}, {safe => 1});
...
# MongoDB::Async::BSON::Binary type
{
    $c->drop;

    my $old = $MongoDB::Async::BSON::use_binary;
    $MongoDB::Async::BSON::use_binary = 0;

    my $str = "foo";
    my $bin = {bindata => [
                   \$str,
                   MongoDB::Async::BSON::Binary->new(data => $str),
                   MongoDB::Async::BSON::Binary->new(data => $str, subtype => MongoDB::Async::BSON::Binary->SUBTYPE_GENERIC),
                   MongoDB::Async::BSON::Binary->new(data => $str, subtype => MongoDB::Async::BSON::Binary->SUBTYPE_FUNCTION),
                   MongoDB::Async::BSON::Binary->new(data => $str, subtype => MongoDB::Async::BSON::Binary->SUBTYPE_GENERIC_DEPRECATED),
                   MongoDB::Async::BSON::Binary->new(data => $str, subtype => MongoDB::Async::BSON::Binary->SUBTYPE_UUID_DEPRECATED),
                   MongoDB::Async::BSON::Binary->new(data => $str, subtype => MongoDB::Async::BSON::Binary->SUBTYPE_UUID),
                   MongoDB::Async::BSON::Binary->new(data => $str, subtype => MongoDB::Async::BSON::Binary->SUBTYPE_MD5),
                   MongoDB::Async::BSON::Binary->new(data => $str, subtype => MongoDB::Async::BSON::Binary->SUBTYPE_USER_DEFINED)]};
    ...
    is($arr[0]->subtype, MongoDB::Async::BSON::Binary->SUBTYPE_GENERIC);
    is($arr[0]->data, $str);
    ...
    $MongoDB::Async::BSON::use_binary = $old;
}
