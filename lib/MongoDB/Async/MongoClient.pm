...
has query_timeout => (
    ...
    isa      => 'Int',
    required => 1,
    default  => sub { return $MongoDB::Async::Cursor::timeout; },
);
...
sub BUILD {
    ...
    eval "use ${_}" # no Any::Moose::load_class becase the namespaces already have symbols from the xs bootstrap
        for qw/MongoDB::Async::Database MongoDB::Async::Cursor MongoDB::Async::OID MongoDB::Async::Timestamp MongoDB::Async::BSON/;

	MongoDB::Async::BSON::read_flags(); # BSON flags cache
    ...
    foreach (@pairs) {
        ...
        $self->_servers->{$_} = MongoDB::Async::MongoClient->new($opts);
...
sub get_database {
    ...
    return MongoDB::Async::Database->new(
        _client     => $self,
        name        => $database_name,
    );
}
...
sub get_master {
    ...
    # auto-detect master
        ...
        if ($master->{'hosts'} && time() > $self->ts) {
            ...
            for (@{$master->{'hosts'}}) {
                if (!$self->_servers->{$_}) {
                    $self->_servers->{$_} = MongoDB::Async::MongoClient->new("host" => "mongodb://$_", %opts);
                }
            }
            $self->ts(time());
        }
...
sub DESTROY {
	...
	if ( $self->{_parent_pool} and !in_global_destruction )
	{ $self->{_parent_pool}->return($self) }
		else
	{ $self->_DESTROY; }
}
...
sub AUTOLOAD {
   ...
   $AUTOLOAD ||= $MongoDB::Async::Connection::AUTOLOAD;
   
    my $db = $AUTOLOAD;
    $db =~ s/.*:://;

	my $sub = eval q/ sub {$_[0]->get_database('/.$db.q/')} /;
...
How many times try to send query. Defaults to C<2>.
...
L<MongoDB::Async::Cursor/slave_okay>.
