...
sub BUILD {
    ...
    eval "use ${_}" # no Any::Moose::load_class becase the namespaces already have symbols from the xs bootstrap
        for qw/MongoDB::Async::Database MongoDB::Async::Cursor MongoDB::Async::OID MongoDB::Async::Timestamp/;
    ...
    foreach (@pairs) {
        ...
        $self->_servers->{$_} = MongoDB::Async::Connection->new($opts);
...
sub _get_max_bson_size {
    ...
    if ($buildinfo->{'maxBsonObjectSize'}) {
...
sub get_master {
    ...
    # auto-detect master
        ...
        if ($master->{'hosts'} && time() > $self->ts) {
            ...
            for (@{$master->{'hosts'}}) {
                if (!$self->_servers->{$_}) {
                    $self->_servers->{$_} = MongoDB::Async::Connection->new("host" => "mongodb://$_", auto_connect => 0);
                }
            }
            $self->ts(time());
        }
...
sub _old_stupid_paired_conn {
    ...
    # check the left host
    eval {
        $left = MongoDB::Async::Connection->new("host" => $self->left_host, "port" => $self->left_port, timeout => $self->timeout);
    };
    ...
    eval {
        $right = MongoDB::Async::Connection->new("host" => $self->right_host, "port" => $self->right_port, timeout => $self->timeout);
    };
    if (!($@ =~ m/couldn't connect to server/)) {
