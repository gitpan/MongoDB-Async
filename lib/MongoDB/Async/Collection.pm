...
sub new {
	...
	$self->{full_name} = $self->{_database}{name}.'.'.$self->{name};
...
sub to_index_string {
    ...
    if (ref $keys eq 'ARRAY' ||
        ...
        die 'expected Tie::IxHash, hash, or array reference for keys';
    }
...
sub insert {
    my ($id) = $_[0]->batch_insert([$_[1]], $_[2]);
    return $id;
}
...
sub batch_insert {
    ...
    die 'not an array reference' unless ref $object eq 'ARRAY';
    ...
    my ($insert, $ids) = MongoDB::Async::write_insert( ($self->{_database}{name}.'.'.$self->{name}) , $object, $add_ids);
...
sub update {
    ...
    my $update = MongoDB::Async::write_update($self->{full_name}, $query, $object, $flags);
    if ($opts->{safe}) {
...
sub remove {
	...
	$options = { just_one => $options || 0 } unless (ref $options eq 'HASH');
    ...
    my $remove = MongoDB::Async::write_remove(
		$self->{full_name},
		$query,
		(exists $options->{just_one} ? $options->{just_one} : 0)
	);
	
    return $self->_make_safe($remove) if ($options->{safe});
...
sub ensure_index {
    ...
    my $ns = ($self->{_database}{name}.'.'.$self->{name});
...
sub _make_safe {
    ...
    my ($query, $info) = MongoDB::Async::write_query($db.'.$cmd', 0, 0, -1, $last_error);

    $conn->send("$req$query");

    my $cursor = MongoDB::Async::Cursor->new(_ns => $info->{ns}, _connection => $conn, _query => {});
    $cursor->_init;
...
sub save { # self, doc, options
    if (exists $_[1]->{_id}) {
        $_[2]->{'upsert'} = 1;
		
        return $_[0]->update({_id => $_[1]->{'_id'}}, $_[1], $_[2]);
    }
    else {
        return $_[0]->insert($_[1], $_[2]);
    }
}
...
sub DESTROY {}; # for prevent AUTOLOAD
