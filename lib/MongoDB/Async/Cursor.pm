...
sub _do_query {
    ...
    my ($query, $info) = MongoDB::Async::write_query($self->_ns, $opts, $self->_skip, $self->_limit, $self->_query, $self->_fields);
    $self->{_request_id} = $info->{request_id};

    $self->{_connection}->send($query);
	
	# Weird shit happens with perl stack if we call perl from XS and try to switch Coro threads. So call recv from Cursor.xs
...
=head2 data
    ...
    my $objects = $cursor->data;

Same as ->all, but returns array ref 
